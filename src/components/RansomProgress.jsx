    ...
    <div className={`bg-white rounded-lg shadow-xl p-6 ${isCompleted ? 'border-4 border-green-500' : ''}`}>
      <div className="text-center mb-4">
        <h2 className="text-2xl font-bold text-[#63006A] mb-2">
          {isCompleted ? '🎉 Vœu exaucé !' : '🕊️ Grues réunies pour le vœu'}
        </h2>
        {config.ransomMessage && (
          <p className="text-gray-600 italic">{config.ransomMessage}</p>
        )}
      </div>
      ...
      <div className="text-center">
        ...
        {!isCompleted && (
          <p className="text-sm text-gray-600 mt-1">
            Plus que <Triceracoin amount={config.ransomGoal - effectiveTotal} size="sm" textColor="text-[#63006A]" /> pour exaucer le vœu !
          </p>
        )}
        ...
        {globalOffset !== 0 && (
          <div className={`mt-3 p-2 rounded-lg inline-block ${
            ...
            <p className={`text-sm font-bold ${
              ...
              {globalOffset < 0 ? '🦹 Voleur' : '🎅 Donateur'} : {globalOffset > 0 ? '+' : ''}{globalOffset} 🕊️
            </p>
          </div>
        )}
      </div>
      ...
      {isCompleted && (
        <div className="mt-4 p-4 bg-green-100 rounded-lg text-center">
          <p className="text-green-800 font-bold text-lg">
            ✨ Le vœu des mariés est exaucé ! Merci à toutes les équipes ! ✨
          </p>
        </div>
      )}
    </div>
